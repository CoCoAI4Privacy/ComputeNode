...
from bs4 import BeautifulSoup, Comment, NavigableString, Doctype, ProcessingInstruction
...
class DataPipeline:
    MODEL_PATH = "../data/policies_model_300.bin"
    ...
    def __init__(self, embedding="..\\data\\policies_model_300.bin", edge_selection=None):
    ...
    def process_url(self, url: str):
        ...
        if html != None:
            clean_text, segments, vector_segments, segment_graphs = self.process_html(
                html)
            return (html, clean_text, segments, vector_segments, segment_graphs)
        else:
            return None
    ...
    def process_text(self, text: str):
        ...
        segment_graphs = [self.graph_builder.build(
            segment) for segment in segments]
        return (text, segments, vectors, segment_graphs)
    ...
    def vectorize_segments(self, segments: list):
        ...
        for segment in segments:
            ...
            for word in segment.split():
                vectorized_segment.append(self.ft.get_word_vector(word))
            vectorized.append(vectorized_segment)
        return vectorized

    # Private helper methods
    def _find_recursive(self, root: BeautifulSoup, name: str):
        children = []
        if type(root) is not NavigableString and type(root) is not Comment and type(root) is not Doctype and type(root) is not ProcessingInstruction:
